...
void disk_read_bpb(struct bios_pram_block *bpb)
{
    ...
    bpb->fat_start_offset=bpb->rsvd_sec_cnt*bpb->byte_per_sec;
    bpb->root_start_offset=bpb->fat_start_offset+(bpb->sec_per_fat*bpb->num_fats)*bpb->byte_per_sec;
    bpb->data_start_offset=bpb->root_start_offset+bpb->root_ent_cnt*32;
    bpb->clus_size=bpb->sec_per_clus*bpb->byte_per_sec;
}
